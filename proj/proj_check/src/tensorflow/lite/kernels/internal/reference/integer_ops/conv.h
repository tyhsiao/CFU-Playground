...
inline void ConvPerChannel(
  ...
  cfu_op0(/* funct7= */ 5,/* in0= */  input_offset, /* in1= */ 0);
    ...
    for (int out_y = 0; out_y < output_height; ++out_y) {
      ...
      for (int out_x = 0; out_x < output_width; ++out_x) {
        ...
        for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
          ...
          int32_t acc = cfu_op0(/* funct7= */ 4, 0, 0); // resets acc
          for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
            ...
            for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
              ...
              for (int in_channel = 0; in_channel < filter_input_depth; ++in_channel) {
                ...
                uint32_t input_index = Offset(input_shape, batch, in_y, in_x, in_channel + group * filter_input_depth);
                ...
                printf("in_channel = %d, input_val = %ld, acc = %ld, input_index = %ld, filter_index = %ld\n"
                      , in_channel, input_val, acc, input_index, filter_index);
                printf("in_x = %d, in_y = %d, batch = %d, group = %d, out_channel = %d\n", in_x, in_y, batch, group, out_channel);
              }
